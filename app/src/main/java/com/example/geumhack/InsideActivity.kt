...
class InsideActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnext.setOnClickListener {
            if (counter == 0) {
                textview.text =  "- Учитель: Медведь, это же ты!"
            }
            if (counter == 1) {
                textview.text = "- Медведь: о я тебя помню! Мы говорили с тобой про памятники вчера!"
            }
            if (counter == 2) {
                textview.text = "- Учитель: да, ты забыл свою балалайку!"
            }
            if (counter == 3) {
                textview.text =  "Учитель вернул медведю балалайку, а у вас прошел замечательный урок в нашем технопарке. Спасибо за прохождение игры!"
            }
            if (counter == 4) {
